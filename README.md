...
## Конструктор внешнего вида (админка)

Всё визуальное настраивается из админки, с живым предпросмотром прямо в форме:

- **Налаштування сайту** — тема, акцентный цвет (генерирует всю палитру), цвет/прозрачность
  шапки и подвала, скругление углов (`corner_style`), hero, меню, анимации.
- **Стили элементов** (`Element styles`) — позиция (сдвиг X/Y), размер (масштаб, шрифт,
  ширина), цвет текста/фона, прозрачность, выравнивание, радиус, отступы, эффект
  (float/pulse/fade-in/slide-up), скрытие и произвольный CSS — для любого элемента
  (шапка, hero, карточки, футер, …) на всём сайте или на конкретной странице.
  Редактируемый элемент подсвечивается в предпросмотре пунктиром.
- **Фоны страниц** (`Page backgrounds`) — на весь сайт или на страницу: статичное фото
  (позиция/заполнение) **или анимированный фон**: градиент, аврора, волны, частицы,
  пузыри, снег, звёзды — либо свой (тип «Custom», JSON-конфиг
  [tsparticles](https://particles.js.org)). Цвета (по умолчанию — акцент темы), скорость,
  приглушение. Снять галочку «Active» — фон убирается совсем.
